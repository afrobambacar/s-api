...
exports.index = function (req, res) {
  ...
  var promise = User.find({}, '-salt -password').sort({ _id: -1 }).skip(start).limit(count).exec();
...
exports.profile = function (req, res, next) {
  ...
  var promise = User.findById(id).select('-salt -password').exec();
...
function resize(params) {
  ...
  var tasks = versions.map(function (version) {
    return new Promise(function (resolve, reject) {
      sharp(params.Body).resize(version, version).jpeg().quality(95).toBuffer(function (err, buffer, info) {
        if (err) {
          reject(err);
        } else {
          resolve({
...
exports.updateProfileImage = function (req, res) {
  ...
  Promise.resolve(params).then(resize).then(putObject).then(function (data) {
